...
mod tests {
    ...
    #[test]
    fn register_claude_mcp_server_updates_stale_entry() {
        let tmp = tempfile::tempdir().unwrap();
        let path = tmp.path().join("settings.json");

        // Write a stale config with old binary path
    ...
    #[test]
    fn register_codex_mcp_server_updates_stale_entry() {
        let tmp = tempfile::tempdir().unwrap();
        let path = tmp.path().join("config.toml");
    ...
    #[test]
    fn register_copilot_mcp_server_updates_stale_entry() {
        let tmp = tempfile::tempdir().unwrap();
        let path = tmp.path().join("mcp.json");
    ...
    #[test]
    fn register_opencode_mcp_server_updates_stale_entry() {
        let tmp = tempfile::tempdir().unwrap();
        let path = tmp.path().join("opencode.json");
