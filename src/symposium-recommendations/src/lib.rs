...
fn serialize_when(when: &When) -> Result<Vec<String>> {
    ...
    if let Some(ref path) = when.file_exists {
        lines.push(format!(
            "when.file-exists = \"{}\"",
    ...
    if let Some(ref paths) = when.files_exist {
        let arr = paths
            .iter()
    ...
    if let Some(ref crate_name) = when.using_crate {
        lines.push(format!(
            "when.using-crate = \"{}\"",
    ...
    if let Some(ref crate_names) = when.using_crates {
        let arr = crate_names
            .iter()
    ...
    if let Some(ref conditions) = when.any {
        let arr = conditions
            .iter()
    ...
    if let Some(ref conditions) = when.all {
        let arr = conditions
            .iter()
            .map(|w| serialize_when_inline(w))
            .collect::<Result<Vec<_>>>()?
