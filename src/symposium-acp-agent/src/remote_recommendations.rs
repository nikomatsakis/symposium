...
pub async fn load_recommendations(config_paths: &ConfigPaths) -> Result<Recommendations> {
    ...
    let remote_toml = load_remote_with_cache(config_paths).await?;
    let mut combined = Recommendations::from_toml(&remote_toml)
        .context("Failed to parse remote recommendations")?;

    // 2. Load user's local recommendations if present and merge
    if let Some(local_toml) = load_local_recommendations(config_paths)? {
        let local_recs = Recommendations::from_toml(&local_toml)
            .context("Failed to parse local recommendations")?;
...
async fn load_remote_with_cache(config_paths: &ConfigPaths) -> Result<String> {
...
async fn fetch_remote_recommendations() -> Result<String> {
    tracing::debug!(
        "Fetching recommendations from {}",
        REMOTE_RECOMMENDATIONS_URL
    );

    let response = reqwest::get(REMOTE_RECOMMENDATIONS_URL)
        .await
    ...
    // Validate that it parses correctly before caching
...
fn load_local_recommendations(config_paths: &ConfigPaths) -> Result<Option<String>> {
    ...
    Recommendations::from_toml(&content).with_context(|| {
        format!(
...
pub fn local_recommendations_path(config_paths: &ConfigPaths) -> std::path::PathBuf {
    config_paths
        .root()
        .join("config")
        .join(LOCAL_RECOMMENDATIONS_FILENAME)
