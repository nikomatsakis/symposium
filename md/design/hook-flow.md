...
2. **Dispatch to plugin hooks** — for each enabled plugin that defines a hook handler for the incoming event:
   - **Skip native hooks**: if the plugin declares a native hook for the current agent (i.e., `format` matches the agent), skip it — the agent already invoked it directly via its own configuration. See [Hooks](./hooks.md) for the conceptual model.
   - **Deliver symposium-format hooks**: for plugins with `format = "symposium"` (or no native hook for this agent):
     - Convert the agent's wire-format input to symposium canonical format.
     ...
     - Pass symposium canonical JSON on stdin to the plugin's hook.
