use symposium::hook::{HookPayload, PostToolUsePayload, PreToolUsePayload};

#[tokio::test]
async fn pre_tool_use_builtin_empty() {
    let ctx = symposium_testlib::with_fixture(&["plugins0"]);
    let payload = HookPayload::from(PreToolUsePayload {
...
async fn post_tool_use_records_bash_activation() {
    ...
    let payload = HookPayload::from(PostToolUsePayload {
        tool_name: "Bash".to_string(),
